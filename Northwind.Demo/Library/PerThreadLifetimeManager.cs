...
using IUntilityDisposible = GenericCodes.Core.IDisposable;
namespace Northwind.Demo.Library
{
    /// <summary>
    ...
    public class PerThreadLifetimeManager : LifetimeManager
    {
        ...
        public override object GetValue()
        {
            ...
            object result;
            values.TryGetValue(this.key, out result);
            if (result != null && result is IUntilityDisposible)
                if (((IUntilityDisposible)result).IsDisposed) // to check if it is disposed or not.
                    result = null;
            return result;
        }
